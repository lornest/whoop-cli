...
func NewSleepModel(client *whoop.Client) SleepModel {
	return SleepModel{client: client, loading: true}
}
...
func (m SleepModel) View() string {
	...
	latest := m.sleep.Records[0]
	var stagesView string
	if latest.Score != nil {
		s := latest.Score.StageSummary
		...
		stagesView = lipgloss.JoinVertical(lipgloss.Left,
			style.LabelStyle.Render("Last Night's Stages"),
	...
	if latest.Score != nil {
		metricsStr = lipgloss.JoinHorizontal(lipgloss.Top,
			components.MetricCard("Performance",
				util.FormatPercent(latest.Score.SleepPerformancePercentage), style.ColorGreen, 22),
			"  ",
			components.MetricCard("Efficiency",
				util.FormatPercent(latest.Score.SleepEfficiencyPercentage), style.ColorCyan, 22),
			"  ",
			components.MetricCard("Resp Rate",
				fmt.Sprintf("%.1f/min", latest.Score.RespiratoryRate), style.ColorBlue, 22),
		)
	}

	// 7-day table
	tableHeader := lipgloss.NewStyle().Foreground(style.ColorDim).Bold(true).Render(
		fmt.Sprintf("  %-12s %-10s %-10s %-10s %-8s", "Date", "In Bed", "Perf %", "Efficiency", "Nap"))
	...
	for _, rec := range m.sleep.Records {
		...
		tableRows += fmt.Sprintf("\n  %-12s %-10s %-10s %-10s %-8s", date, inBed, perf, eff, nap)
	}

	content := lipgloss.JoinVertical(lipgloss.Left,
		title, "", metricsStr, "", stagesView, "", tableHeader, tableRows)
